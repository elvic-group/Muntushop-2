...
class WhatsAppHandler {
  async handleIncomingMessage(webhookData) {
    try {
      ...
      // Safely extract message text - ONLY process if actual text message exists
      let message = "";
      if (
        messageData &&
      ...
      // CRITICAL: Skip processing if no actual text message (prevents unwanted messages)
      // This prevents responding to status updates, read receipts, etc.
  ...
  async routeMessage(user, message) {
    ...
    // CRITICAL: Don't send messages for empty/invalid messages
    // This prevents automatic menu sends from empty webhooks
    if (!message || typeof message !== "string" || message.trim().length === 0) {
      console.log(`⚠️ Skipping routing - empty or invalid message from ${user.phone}`);
      return;
    }
  ...
  async sendServiceMenu(user, service) {
    try {
      ...
      await this.sendMessage(
        user.phone,
        "Sorry, an error occurred. Type MENU to continue."
      );
    }
  }
    ...
    try {
      ...
      switch (user.current_service) {
        ...
        default:
          await this.sendMessage(
            user.phone,
            "Service not implemented yet. Type MENU to continue."
          );
      }
    } catch (error) {
