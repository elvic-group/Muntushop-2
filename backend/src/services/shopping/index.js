...
class ShoppingService {
  ...
  async handleMenuSelection(user, message) {
    ...
    if (isNaN(option)) {
      await this.sendMessage(
        user.phone,
        "Invalid option. Please reply with a number (1-8)."
      );
      return;
    }
    ...
    if (option >= 1 && option <= 5) {
      await this.updateUserStep(user.id, "category", {
  ...
  async showProducts(user, category) {
    ...
    await this.updateUserStep(user.id, "products", {
      category,
  ...
  async handleProductSelection(user, message) {
    ...
    if (isNaN(option)) {
      await this.sendMessage(
        user.phone,
        "Invalid option. Please reply with a number."
      );
      return;
    }
    ...
    if (user.session_data) {
      try {
        session =
          typeof user.session_data === "string"
  ...
  async handleProductAction(user, message) {
    ...
    if (!productId) {
      await this.sendMessage(
        user.phone,
        "Product not selected. Please try again."
      );
      return;
    }
  ...
  async handleCartAction(user, message) {
    ...
    if (option === 1) {
      ...
      if (!cart || !cart.items || cart.items.length === 0) {
        await this.sendMessage(
          user.phone,
          "Your cart is empty. Type MENU to browse products."
        );
        return;
      }
  ...
  async handleAddress(user, address) {
    ...
    if (!address || address.trim().length === 0) {
      await this.sendMessage(
        user.phone,
        "Please provide a valid address. Type CANCEL to go back."
      );
      return;
    }
    ...
    if (address.trim().length < 5) {
      await this.sendMessage(
        user.phone,
        "Please provide a complete address (at least 5 characters). Type CANCEL to go back."
      );
      return;
    }

    // Save address
    await this.updateUserStep(user.id, "checkout_payment", {
...
Type PAID to confirm.
  ...
  // Create order without confirmation (for pending payment)
  async createOrderPending(user, cart) {
  ...
  async createOrderConfirmed(user, cart, orderId = null) {
    ...
    if (orderId) {
      ...
      // Create new order
      order = await this.createOrderPending(user, cart);
    }
